...
class ObjectViewedManger(models.Manager):
...
class ObjectViewed(models.Model):
    user            = models.ForeignKey(User, blank=True, null=True) # actual user instance 
    ip_address      = models.CharField(max_length=220, blank=True, null=True)
    content_type    = models.ForeignKey(ContentType) # User, Course, Order, Cart, Address
    object_id       = models.PositiveIntegerField()  # user id, course id, order id
    content_object  = GenericForeignKey('content_type', 'object_id')
    timestamp       = models.DateTimeField(auto_now_add = True)

    objects = ObjectViewedManger()
...
#for ending all other session and continuing with current session only
